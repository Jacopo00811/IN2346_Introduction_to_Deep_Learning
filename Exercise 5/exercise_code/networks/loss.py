...
class CrossEntropyFromLogits(Loss):
    ...
    def forward(self, y_out, y_truth, reduction='mean'):
        ...
        # Each integer value is transformed into a vector with:
        # 0 -> [1, 0, 0]
        # 1 -> [0, 1, 0]
        # 2 -> [0, 0, 1]
        N, C = y_out.shape
        ...
        # The result is that each row of y_truth_one_hot has a 1 in the column corresponding 
        # to the correct class label, and 0 in all other columns. This is a one-hot encoding
        # of the class labels
        ...
        # loss = loss.sum(axis=1).mean() calculates the mean of the 
        # column sums of the loss array
