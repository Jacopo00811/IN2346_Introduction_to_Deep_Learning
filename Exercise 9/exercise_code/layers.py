...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # Reshape gamma and beta to broadcast over spatial dimensions
    gamma_reshaped = gamma.reshape(1, -1)
    beta_reshaped = beta.reshape(1, -1)

    # Perform batch normalization on reshaped data
    out_reshaped, cache = batchnorm_forward(x_reshaped, gamma_reshaped, beta_reshaped, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # Reshape gamma and beta to broadcast over spatial dimensions
    gamma_reshaped = cache[3].reshape(1, -1)
    beta_reshaped = cache[2].reshape(1, -1)

    # Perform batch normalization backward on reshaped data
    dx_reshaped, dgamma_reshaped, dbeta_reshaped = batchnorm_backward(dout_reshaped, cache)

    # Reshape the gradients back to the original spatial dimensions
    dx = dx_reshaped.reshape(N, H, W, C).transpose(0, 3, 1, 2)
    dgamma = dgamma_reshaped.reshape(gamma_reshaped.shape)
    dbeta = dbeta_reshaped.reshape(beta_reshaped.shape)
